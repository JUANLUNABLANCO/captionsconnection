    ...
    if (req.body.email) {
        ...
        Client.findOne({ email: req.body.email }, (err, email_finded) => {
            if (err) {
                _PRINT.Console(
                    "500 ERROR BD",
                    "clientController.checkEmailExists(Client.findOne())",
                    err
                );
                // error de servidor
        ...
        _PRINT.Console(
            ...
            "clientController.checkEmailExists(if(req.body.email))",
            "El body no es correcto, no llega req.body.email"
        );
        ResManager.sendError(
            res,
            412,
            "¡BODY NO válido!",
            "checkChannelNameExists: if(req.body.email==false)",
    ...
    if (req.body.email) {
        ...
        Client.findOne({ email: email }, (err, client_finded) => {
            if (err) {
                _PRINT.Console(
                    "500 ERROR BD",
                    "clientController.Register(Client.findOne())",
                    err
                );
                ...
                if (client_finded) {
                    ...
                    newClient.save((err, client_created) => {
                        ...
                        if (err) {
                            _PRINT.Console(
                                "500 ERROR BD",
                                "clientController.Register(newClient.save())",
                                "ese email ya existe"
                            );
                            ResManager.sendError(
                                res,
                                ...
                                (err, user_updated) => {
                                    if (err) {
                                            ...
                                            (err, client_updated) => {
                                                if (err) {
                                                    ...
                                                    ResManager.sendOkData(
                                                        res,
                                                        200,
                                                        "Nuevo cliente creado. Usuario exite, cliente actualizado",
                                                        client_created._id,
                                                        "clientController.Register(newClient.save())",
                                                        {
                                        ...
                                        ResManager.sendOkData(
                                            res,
                                            200,
                                            "Nuevo cliente creado. No existe usuario",
                                            client_created._id,
                                            "clientController.Register(newClient.save())",
                                            { client_created: client_created }
                                        );
        ...
        ResManager.sendError(
            res,
            412,
            "El body no existe o algún campo falta",
