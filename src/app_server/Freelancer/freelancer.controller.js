...
freelancerController.checkEmailExists = (req, res) => {
    ...
    if (req.body.email ) { // req.body.email
        ...
        Freelancer.findOne({email: req.body.email}, (err, email_finded) =>{ 
            if (err){
                _PRINT.Console('500 ERROR BD', 'freelancerController.checkEmailExists(Freelancer.FindOne())', err);
                // error de servidor
        ...
        _PRINT.Console('412 Preconditions Failed', 'freelancerController.checkEmailExists(if(req.body == false))', 'BODY NO VALIDO');
        ResManager.sendError( res, 412, "¡El body no existe o no es válido!", "checkEmailExists: else(req.body)", "¡Preconditions falilled!", "Not err BD" );
    }
}
// ///////////////////////////////////////////////////////////////////
// ///////////////////////////////////////////////////////////////////
...
freelancerController.Register = (req, res) =>{
    ...
    // console.log("validación correcta");
    // ######################## viene el req.body
    if (req.body.email && req.body.nativeLanguage && req.body.languages && req.body.habilities && req.body.country){
        // console.log(req.body);
        const email = req.body.email;
        // ######################## buscamos es email
        Freelancer.findOne({email: email}, (err, email_finded) =>{ 
            if (err){
                _PRINT.Console('500 ERROR BD', 'freelancerController.Register(Freelancer.FindOne())', err);
                ...
                if ( email_finded  ){
                    ...
                    _PRINT.Console('newFreelancer','freelancerController.Register(Freelancer.findOne(if(email_finded==false)))', newFreelancer);
                    // ######################## creamos el freelancer
                    newFreelancer.save((err, freelancer_created) =>{ 
                        if (err) {
                            _PRINT.Console('500 ERROR BD', 'freelancerController.Register(newFreelancer.save())', err);
                            ...
                            User.findOne({email: freelancer_created.email}, (err, user_finded)=>{
                                if (err){
                                    _PRINT.Console('500 ERROR BD', 'freelancerController.Register(User.findOne())', err);
                                    ResManager.sendError( res, 500, "", "freelancerController.Register(User.findOne())", "¡ERROR: BD falló!", err );
                                } else if (user_finded) {
                                    ...
                                    Freelancer.findOneAndUpdate(
                                        ...
                                        (err, freelancer_updated)=>{
                                            if (err){
                                                ...
                                                // OK: ##########################################  
                                                // OK: ##########################################
                                                _PRINT.Console('200 Freelancer created and updated', 
                                                    'freelancerController.Register(User.findOne(freelancer_updated==true))', freelancer_updated);
                                                ResManager.sendOkData(res, 200, "Nuevo Freelancer creado y actualizado", 
                                    ...
                                    // OK: ##########################################  
                                    // OK: ##########################################
                                    _PRINT.Console('200 Freelancer created', 'freelancerController.Register(User.findOne())', freelancer_created);
    ...
    // ######################## no viene el req.body
        ...
        _PRINT.Console('412 Preconditions Failed', 'freelancerController.Register(if(req.body==false))', 'BODY NO VÁLIDO');
        ResManager.sendError( res, 412, "El body no existe o es erróneo!", "Register: else(req.body)", "¡Preconditions failled!", "Not err BD" );
    }
    // ######################## no viene el req.body
};
