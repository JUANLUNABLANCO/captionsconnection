...
const ChannelSchema =  new Schema ({
    channelType:        {type: String, required: "El 'channelType' es requerido", enum: CB.channelType},
    clientEmail:        {
        type: String, 
        required: "El 'clientEmail' es requerido",
        match: [CB.regExEmail, "El 'clientEmail' no es válido "], 
        index: { unique: false }, 
        lowercase: true, 
        trim: true},
    channelName:        {
        ...
        match: [CB.regExChName, "El 'channelName' no es válido."],
        minlength: [3, "demasiado corto"], 
        index: { unique: true }, 
        lowercase: true, 
        trim: true},
    channelLanguage:            {type: String, required: true, enum: CB.channelLanguage},
    channelCategory:            {type: String, required: true, enum: CB.channelCategory},
    ...
    // ##################### introducidos de segundas, cuando se cree el cliente
    client:             { type: Schema.Types.ObjectId, ref: 'client', required: false},// el dueño del canal
...
ChannelSchema.post('save', (doc) => {
    ...
    if (_ENV == 'development'){
        ...
        console.log('clientType: ',  CB.arrClientTypes[CB.arrChannelTypes.indexOf( doc.channelType)]);
    }
});
