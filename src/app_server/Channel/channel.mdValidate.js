...
channelForm_CheckNameExists = (req, res, next) => {
    ...
    if (!req.body) {
        ...
        sendErrorMDValidate(
            res,
            "el 'channelName' no cumple con los caracteres permitidos. 'a-z A-Z 0-9 ñ Ñ áéíóúÁÉÍÓÚÜü_-@#$.'"
        );
    } else {
...
channelForm_ChannelSave = (req, res, next) => {
    ...
    if (!req.body) {
        ...
        req.body.clientInterestingNow == null
    ) {
        console.log(req.body);
        // no existe algun campo
        sendErrorMDValidate(
            res,
            "Algún campo requerido no llegó. Revisa los datos"
        );
    } else if (req.body.channelName.length < 3) {
        ...
        sendErrorMDValidate(
            res,
            "el 'clientEmail' no cumple con los caracteres permitidos, para un email válido"
        );
    } else if (arrChannelTypes.indexOf(req.body.channelType) < 0) {
        // enum: ["YOUTUBER"]
        sendErrorMDValidate(
            res,
            "el 'channelType' no coincide con los valores permitidos"
        );
    } else if (arrChannelLanguages.indexOf(req.body.channelLanguage) < 0) {
        // enum: [ 'ES', 'EN', 'FR', 'PT', 'other']
        sendErrorMDValidate(
            res,
            "el 'channelLanguage' no coincide con los valores permitidos"
        );
        ...
        sendErrorMDValidate(
            res,
            "el 'channelCategory' no coincide con los valores permitidos"
        );
    } else if (
        req.body.clientInterestingNow != true &&
        req.body.clientInterestingNow != false
    ) {
        // true | false
        sendErrorMDValidate(
            res,
            "el 'clientInterestingNow' debe ser true or false"
        );
    } else {
        req.body.channelName = req.body.channelName.trim().toLowerCase();
