...
channelController.checkChannelNameExists = (req, res) => {
    ...
    if (req.body.channelName) { // req.body.channelName
        ...
        Channel.findOne({channelName: req.body.channelName}, (err, channel_finded) =>{ 
            if (err){
                _PRINT.Console('500 ERROR BD', 'channelController.checkChannelNameExists(Channel.findOne())', err);
                // error de servidor
        ...
        _PRINT.Console('412 Preconditions Failed', 'channelController.checkChannelNameExists(if(req.body == false))', 'no llegan datos correctos');
        ResManager.sendError( res, 412, "¡El body no existe o no es correcto!", "checkChannelNameExists: else(req.body)", "¡Preconditions falilled!",  "Not err BD" );
    }
}
// ############ http://localhost:3333/api/channel/check-channel-exists
//////////////////////////////////////////////////////////////////////
...
channelController.youtubeChannelClientRegister = (req, res) =>{
    // ######################## vien el req.body
    if (req.body.channelName  && req.body.clientEmail && req.body.channelType && req.body.channelLanguage && req.body.channelCategory ){
        // _PRINT.Console(req.body);
        const channelName = req.body.channelName;
        // CHANNEL ######################## buscamos es channelName
        Channel.findOne({channelName: channelName}, (err, channel_finded) =>{ 
            if (err){
                _PRINT.Console('500 ERROR BD', 'channelController.youtubeChannelClientRegister(Channel.findOne())', err);
                ...
                if ( channel_finded  ){
                    ...
                    newChannel.save((err, channel_created) =>{ 
                        if (err) {
                            // error de servidor
                            _PRINT.Console('500 ERROR BD', 'channelController.youtubeChannelClientRegister(Channel.save())', err);
                            ...
                            Client.findOneAndUpdate(
                                ...
                                if (err){
                                    // error de servidor
                                    _PRINT.Console('500 ERROR BD', 'channelController.youtubeChannelClientRegister(Client.findOneAndUpdate())', err);
                                    ...
                                    if (client_updated){
                                        _PRINT.Console('if(client_updated == true)', 'channelController.youtubeChannelClientRegister(Client.findOneAndUpdate(client_updated==true))', client_updated);
                                        let reason = "cliente actualizado";
                                        // let _idClient = client_updated._id;
                                        actualizaChannel(client_updated, reason);
                                    }else {
                                        // ######################## creación de un nuevo cliente
                                        _PRINT.Console('if(client_updated == false)', 'channelController.youtubeChannelClientRegister(Client.findOneAndUpdate(client_updated==false))', null );
                                        let customerJourney = channel_created.clientInterestingNow ? clientConfig.arrCustomersJourney[1] : clientConfig.arrCustomersJourney[0]; 
                                        _PRINT.Console('customerJourney', 'channelController.youtubeChannelClientRegister(Client.findOneAndUpdate(if(client_updated==false)))', customerJourney);
                                        ...
                                        _PRINT.Console('newClient', 'channelController.youtubeChannelClientRegister(Client.findOneAndUpdate(if(client_updated==false)))',newClient);
                                        newClient.save((err, client_created) =>{ 
                                            // HASK: MIRAR CUAL ES MEJOR ese o este:  Channel.create(new_channel, (err, channel_created) =>{
                                            if (err) {
                                                _PRINT.Console('500 ERROR BD', 'channelController.youtubeChannelClientRegister(Client.findOneAndUpdate(if(newClient.save())))', err);
                                                // error de servidor
                                                ResManager.sendError( res, 500, "","youtubeChannelClientRegister: newClient.save()", "¡ERROR: BD falló!",  err );
                                            } else {
                                                _PRINT.Console('client_created', 'channelController.youtubeChannelClientRegister(newClient.save())', client_created );
                                                // clientUpdatedOrCreated = client_created;
                                                // _idClient = client_created._id;
                                                let reason = "cliente creado, no actualizado";
                                                actualizaChannel(client_created, reason);
                                            }
                                        });
                                        // ######################## creación de un nuevo cliente
                                    }
                                    // CHANNEL ####################### funcion que debe ejecutarse en cualquiera de los dos casos: client_updated  or client_cretaed
                                    function actualizaChannel (clientUpdatedOrCreated, reason){
                                            _PRINT.Console("reason", "channelController.youtubeChannelClientRegister(actualizaChannel)", reason);
                                            // ############################ actualizacion de channel_created
                                            // console.log(_idClient);
                                            Channel.findOneAndUpdate(
                                                {_id: channel_created._id},                     // encuentra este 
                                                {_idClient: clientUpdatedOrCreated._id},                // nuevos datos a actualizar
                                                {new: true},                                    // para recoger el nuevo actualizado
                                                function(err, channel_updated){
                                                    if (err) {
                                                        _PRINT.Console('500 ERROR BD', 'channelController.youtubeChannelClientRegister(Channel.findOneAndUpdate())', err);
                                                        ...
                                                        User.findOne({email: channel_created.clientEmail}, (err, user_finded)=>{
                                                                if (err) {
                                                                    _PRINT.Console('500 ERROR BD', 'channelController.youtubeChannelClientRegister(User.findOneAndUpdate())', err);
                                                                    ResManager.sendError( res, 500, "", "youtubeChannelClientRegister: Channel.findOneAndUpdate()", "¡ERROR: BD falló!",  err );
                                                                } else if (user_finded) {
                                                                    // USER ############################### actualizar el {_idUser: user_finded._id, regsitered: true}
                                                                    Client.findOneAndUpdate(
                                                                        {email: channel_created.clientEmail},
                                                                        {_idUser: user_finded._id, registered: true }, 
                                                                        {new: true}, 
                                                                        (err, client_updated)=>{
                                                                        if (err) {
                                                                            _PRINT.Console('500 ERROR BD', 'channelController.youtubeChannelClientRegister(Client.findOneAndUpdate())', err);
                                                                            ResManager.sendError( res, 500, "", "channelController.youtubeChannelClientRegister(Client.findOneAndUpdate())", "¡ERROR: BD falló!",  err );
                                                                            ...
                                                                            ResManager.sendOkData(res, 200, "¡Nuevo 'Canal' creado!. cliente actualizado, canal actualizado y cliente actualizado", 
                                                                                channel_created._id, 'from: channelController.youtubeChannelClientRegister(Channel.findOneAndUpdate(user_updated==true))', {
                                                                    ...
                                                                    // OK: ##########################################
                                                                    // sendOkData = (res, status, message,  data_public, origin, data_private )
                                                                    ResManager.sendOkData(res, 200, "¡Nuevo 'Canal' creado!. cliente actualizado y canal actualizado", channel_created._id, 
    ...
    // ######################## no viene el req.body
        ...
        _PRINT.Console('412 Preconditions Failed', 'channelController.youtubeChannelClientRegister(if(req.body == false))', "El body no existe")
