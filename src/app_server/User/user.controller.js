...
userController.checkEmailExists = (req, res) => {
    ...
    if (req.body.email) { // req.body.channelName
        ...
        User.findOne({email: req.body.email}, (err, user_finded) =>{ 
            if (err){
                _PRINT.Console('500 ERROR BD', 'userController.checkEmailExists(User.findOne())', err);
                // error de servidor
                ResManager.sendError( res, 500, "", "userController.checkEmailExists(User.findOne())", "¡ERROR: BD falló!", err );
            } else {
        ...
        _PRINT.Console('412 Preconditions Failed', 'userController.checkEmailExists(if(req.body == false))', 'no llegan datos correctos');
        ResManager.sendError( res, 412, "¡El body no existe o no es correcto!", "userController.checkEmailExists(if(req.body==false))", "¡Preconditions falilled!",  "Not err BD" );
    }
}
...
userController.Register = (req, res) =>{
    // ######################## vien el req.body
    // console.log('in userController(): ', req.body);
    if (req.body.name  && req.body.email && req.body.confirmEmail && req.body.password  && req.body.role ){
        // _PRINT.Console(req.body);
        const email = req.body.email;
        // ######################## buscamos ese email
        User.findOne({email: email}, async (err, user_finded) =>{ 
            if (err){
                _PRINT.Console('500 ERROR BD', 'userController.Register(User.findOne())', err);
                // error de servidor: ResManager.sendError( res, error, status, origin, statusMessage, message  )
                ResManager.sendError( res, 500, "", "userController.Register(User.findOne())", "¡ERROR: BD Falló!", err );
            } else {
                // ya habia alguien con ese nombre de canal
                if ( user_finded  ){
                    ResManager.sendError( res, 412, "¡Este email ya existe!", "userController.Register(User.findOne(if(user_finded==true)))", "¡Preconditions Failed!", "Not BD Error" );
                } else {
                    var newUser = new User(req.body);// 
                    // emails coinciden?
                    if (newUser.confirmEmails(req.body.email, req.body.confirmEmail)) {
                        newUser.password =  await newUser.encryptPassword(req.body.password); // encriptación de contraseña
                        // USER ######################## user save
                        newUser.save((err, user_created) =>{ 
                            if (err) {
                                // error de servidor
                                _PRINT.Console('500 ERROR BD', 'userController.Register(newUser.save())', err);
                                ResManager.sendError( res, 500, "", "userController.Register(newUser.save())", "¡ERROR: BD falló!",  err );
                            } else {
                                ////////////////////////////////////////////
                                ...
                                if(user_created.role == R.ROLE_FREELANCER){
                                    ...
                                    Freelancer.findOneAndUpdate(
                                        ...
                                        {_idUser: user_created._id, registered:true},
                                        {new: true},
                                        (err, freelancer_updated)=>{
                                            if (err){
                                                ResManager.sendError(res, 500, 'ERROR. BD falló!', 'userController.Registe(Freelancer.findOneAndUpdate())', 'BD ERROR' );
                                                ...
                                                User.findOneAndUpdate(
                                                    ...
                                                    (err, userRole_updated)=>{
                                                        if (err){
                                                            ResManager.sendError(res, 500, 'ERROR. BD falló!', 'userController.Registe(User.findOneAndUpdate(userRole_update))', 'BD ERROR' );
                                                        }else {

                                                            // FREELANCER ############################################ 
                                                            _PRINT.Console('USER-FREELANCER', 'userController.Register(newUser.save(freelancer_updated==true))', 'USUARIO CREADO. FREELANCER ACTUALIZADO' );
                                                            ResManager.sendOkData(res, 200, "Nuevo usuario de tipo freelancer creado y freelancer actualizado", user_created._id, 
                                                ...
                                                // FREELANCER ############################################ 
                                                _PRINT.Console('USER-FREELANCER', 'userController.Register(newUser.save(freelancer_updated==false))', 'USUARIO CREADO. FREELANCER NO ACTUALIZADO' );
                                                ResManager.sendOkData(res, 200, "Nuevo usuario de tipo freelancer creado. Freelancer no existía!", user_created._id, 
                                                "userController.Register(newUser.save())", user_created );
                                    ...
                                    Client.findOneAndUpdate(
                                        ...
                                        (err, client_updated)=>{
                                            if (err){
                                                ...
                                                User.findOneAndUpdate(
                                                    ...
                                                    (err, userRole_updated)=>{
                                                        if (err){
                                                            ...
                                                            // CLIENT ############################################ 
                                                            _PRINT.Console('USER', 'userController.Register(newUser.save(client_updated==true))', 'USUARIO CREADO. CLIENTE ACTUALIZADO' );
                                                            ResManager.sendOkData(res, 200, "Nuevo usuario de tipo client creado y client actualizado", user_created._id, 
                                                ...
                                                // CLIENT ############################################ 
                                                _PRINT.Console('USER', 'userController.Register(newUser.save(client_updated==false))', 'USUARIO CREADO. CLIENTE NO ACTUALIZADO' );
                                                ResManager.sendOkData(res, 200, "Nuevo usuario de tipo client creado. client no existía!", user_created._id, "userController.Register(newUser.save(client_updated==false))", user_created );
                                                // ######################## actualización de cliente
                                                // CLIENT ############################################
                                            }
                                    });
                                ...
                                // USERISUSER ############################## no hacemos nada
                                    ...
                                    // USER ############################################ 
                                    _PRINT.Console('USER', 'userController.Register(newUser.save())', 'USUARIO CREADO. NI CLIENTE NI FREELANCER. user is only user' );
                                    ResManager.sendOkData(res, 200, "Nuevo usuario creado. No es cliente ni es freelancer", user_created._id, "userController.Register(newUser.save())", user_created );
                                    // ######################## actualización de cliente
                                    // USER ############################################
                                }
                                // USERISUSER ############################## no hacemos nada
                    ...
                    } else { // la pasword y el confirmPassword no son iguales
                        ResManager.sendError(res, 412, "passwords no coinciden", "userController.register(newUser.confirmPasswords(req.body.password, req.body.confirmPassword)==false)", "Preconditions Failed", "not bd error" );
                    }
    ...
    // ######################## no viene el req.body
        ...
        _PRINT.Console('412 Preconditions Failed', 'userController.Register(if(req.body == false))', "el body es inválido")
        ResManager.sendError( res, 412, "BODY NO válido", "userController.Register(if(req.body==true))", "¡Preconditions failled!", "Not err BD" );
    }
    // ######################## no viene el req.body
};
