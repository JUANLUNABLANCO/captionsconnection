...
userForm_userRegister = (req, res, next) =>{
    ...
    if (!req.body){
        ...
        sendErrorMDValidate(res, "el 'email' no cumple con los caracteres permitidos, para un correo válido");
    } else if ( !req.body.email || !req.body.name || !req.body.password || !req.body.confirmEmail) {// required:true
        ...
        if (arrRoles.indexOf(req.body.role) < 0 ) {
            ...
            sendErrorMDValidate(res, "el 'userRole' no coincide con los valores permitidos");
        } else {
            // ROLES: ['user', 'client', 'freelancer']   
            if(req.body.role == arrRoles[2]){           // 'freelancer'
            req.body.role = R.ROLE_FREELANCER; 
            } else if (req.body.role == arrRoles[1]){   // 'client'
